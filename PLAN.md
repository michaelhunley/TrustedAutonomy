...
11. [ ] **Bug C — Incomplete top-level draft summary fields**: `ta draft view` shows generic placeholders in the Summary/Why/Impact header (e.g., `Summary: Changes from goal: v0.13.1.1`, `Why: v0.13.1.1`, `Impact: 11 file(s) changed`) while per-artifact `explanation_tiers` are rich and correct. Root cause: the TA agent calls `ta_pr_build` with minimal filler text in the top-level fields instead of synthesizing a meaningful summary. The `What Changed (N files):` file list renders correctly — only the header fields are the problem. Fix: (a) detect placeholder values in `ta_pr_build` MCP handler and auto-generate top-level summary from artifact descriptions if fields look generic, and/or (b) improve `ta_pr_build` tool description to be prescriptive about `what_changed`/`why` content.
