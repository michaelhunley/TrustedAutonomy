...
### v0.12.3 — Self-Service Operations & Plan Intelligence
<!-- status: pending -->
**Goal**: Make `ta shell` and channel plugins (Discord, etc.) the primary interface for 99% of TA work. The user should rarely need to drop to a raw terminal, read logs manually, or ask an external agent to inspect TA internals. All operational tasks flow through the same pattern: agent interprets intent → proposes action → daemon mediates → user approves.

#### Design Principle
The agent has **read-only** access to TA state (goals, drafts, plan, logs, plugin health). All *writes* — plan edits, goal state changes, config updates — go through the daemon, which marshalls user permission via the existing `deliver_question` / approval flow. The agent never writes directly. This is the same trust model as code changes through staging/drafts, extended to operational tasks.

**Interaction pattern:**
1. User states intent in natural language (shell or Discord)
2. Agent reads relevant TA state, interprets intent, identifies the right action
3. Agent proposes the change with context: what it would do, where, and why
4. Agent asks clarifying questions if intent is ambiguous
5. User approves (or refines)
6. Agent submits the change to the daemon as a staged artifact / draft
7. Daemon applies on approval — same flow as `ta draft apply`

#### Plan Intelligence
Today `ta plan add` creates new phases via an agent session, but cannot add items within existing phases, move items between phases, or reason about plan structure. The user has to manually edit PLAN.md for anything beyond "create a new phase."

1. [ ] **Agent-mediated plan editing**: User states intent ("add goal timeout to monitoring section"), agent reads PLAN.md, proposes item text and placement within the correct phase/section, asks for approval, stages the edit as a draft artifact.
2. [ ] **`ta plan add-item --phase <id> "<description>"`**: CLI entry point that invokes the agent to propose placement within an existing phase. Agent considers section headers, existing item numbering, and cross-references.
3. [ ] **`ta plan move-item --from <phase> --to <phase> --item <N>`**: Relocate items between phases with automatic renumbering. Agent proposes the move with rationale ("item 15 depends on v0.11.0 which ships first").
4. [ ] **`ta plan discuss "<topic>"`**: Agent reads the full plan, identifies relevant phases and items, proposes where new work fits, asks clarifying questions, then stages additions on approval. Supports multi-turn conversation.
5. [ ] **Cross-reference awareness**: When adding or moving items, agent identifies related items in other phases and notes dependencies ("see also v0.12.2 item 8").
6. [ ] **Phase creation via intent**: "We need a phase for compliance audit" → agent proposes phase ID, title, goal statement, problem description, and initial items based on the discussion. Stages as draft.

#### Daemon Observability (Read-Only Agent Access)
The agent in `ta shell` / Discord can interrogate the daemon for operational state. All queries are read-only — no mutations without user approval.

7. [ ] **`ta daemon health`**: Process health, port status, uptime, connected clients, active goal count with PID liveness, plugin status, last error.
8. [ ] **`ta daemon logs [--tail N] [--since <time>] [--level warn]`**: Read daemon log from CLI/shell. Filterable by level and time range. Agent can query this to diagnose issues.
9. [ ] **`ta goal inspect <id>`**: Deep diagnostic view: agent PID alive?, last 20 agent log lines, last event timestamp, staging dir size, draft state and ID, elapsed time, exit code if finished, stderr tail if failed.
10. [ ] **`ta draft inspect <id>`**: Full artifact list with change types, superseded-by chain, approval/denial history with timestamps and reviewer, AI summary.
11. [ ] **`ta status --deep`**: Combined view: daemon health + active goals with health indicators + pending drafts + pending questions + open PRs with CI status. The "one command to understand everything" diagnostic.

#### Goal Diagnostics & Self-Healing
When something goes wrong, the agent should diagnose it through TA queries rather than requiring the user to read kernel logs or inspect process tables.

12. [ ] **Goal failure post-mortem**: On transition to `failed`, daemon captures: last N agent log lines, exit code, stderr, signal (e.g., SIGKILL), system-level errors. Stored on the goal record. Queryable via `ta goal inspect`.
13. [ ] **Pre-flight checks**: Before `ta run`, daemon verifies: agent binary exists and is executable, required env vars set, staging dir writable, no port conflicts, disk space adequate. Reports all failures upfront with actionable messages.
14. [ ] **`ta doctor`**: System-wide diagnostic that checks: daemon reachable, all configured plugins healthy, agent binaries present and signed (macOS), env vars set, disk space, stale goals, orphan staging dirs. Prescriptive output: "Fix: run `codesign --force --sign - <path>`".

#### Plugin Lifecycle

15. [ ] **`ta plugin build <name|all>`**: Build channel/submit plugins from their directories. Handles macOS code re-signing after copy. `ta plugin build discord` → build + install + verify executable.
16. [ ] **`ta plugin status [<name>]`**: Which plugins are installed, version, running (if persistent like Discord listener), last health check, last error.
17. [ ] **`ta plugin logs <name> [--tail N]`**: Read plugin-specific logs (e.g., `.ta/discord-listener.log`).

#### Git/PR Lifecycle Through TA

18. [ ] **`ta pr status [<id>]`**: Show open PRs linked to goals, CI check status, review state, merge readiness. Agent can query this to answer "is my PR ready to merge?"
19. [ ] **`ta pr list`**: All open PRs with goal linkage, phase, and quick status summary.
20. [ ] **Goal → PR linkage in `ta goal status`**: If a goal produced a PR, show PR number, CI status, review state, and merge status.

#### Automatic Staging Cleanup

21. [ ] **Auto-clean staging on `ta draft apply`**: After successful apply, automatically remove the goal's staging directory. Configurable in `daemon.toml`:
    ```toml
    [staging]
    auto_clean = true          # default: true — remove staging after successful apply
    retention_days = 0         # days to keep staging after apply (0 = immediate)
    max_staging_gb = 50        # warn or auto-gc when total staging exceeds this
    ```
    When `auto_clean = false`, staging is retained until `ta goal gc --include-staging`. When `max_staging_gb` is exceeded, daemon logs a warning and optionally runs gc on oldest terminal-state staging dirs.
22. [ ] **Disk space pre-flight**: Before `ta goal start`, check available disk space against estimated staging size (~project size). Warn if disk would drop below 10GB after staging copy. Block if below 2GB.
23. [ ] **`ta gc` unified command**: Single command that runs both `ta goal gc --include-staging` and `ta draft gc`. Shorthand for full cleanup.
