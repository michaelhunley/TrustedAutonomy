...
fn run_vcs_setup(
    ...
    // ── .taignore generation ─────────────────────────────────────
    generate_taignore(project_root, dry_run)?;
...
/// Detect project type and auto-generate a project-appropriate `.taignore`.
///
/// Detects known project types based on presence of key files/directories
/// and generates patterns that prevent large build artifact directories from
/// being copied into staging. Merges with any existing `.taignore` entries —
...
/// - Go (go.mod) → vendor/
/// - Python (pyproject.toml/requirements.txt) → __pycache__/, .venv/, venv/
/// - Unreal Engine (*.uproject) → Binaries/, Intermediate/, Saved/, DerivedDataCache/
/// - Java/Gradle (build.gradle) → build/, .gradle/
/// - Java/Maven (pom.xml) → target/
fn generate_taignore(project_root: &std::path::Path, dry_run: bool) -> anyhow::Result<()> {
    let taignore_path = project_root.join(".taignore");

    // Detect project types.
    let mut new_patterns: Vec<&str> = Vec::new();

    if project_root.join("Cargo.toml").exists() {
        new_patterns.extend(&["target/"]);
    }
    if project_root.join("package.json").exists() {
        new_patterns.extend(&["node_modules/", ".next/", "dist/", ".pnp/", ".yarn/"]);
    }
    if project_root.join("go.mod").exists() {
        new_patterns.extend(&["vendor/"]);
    }
    if project_root.join("pyproject.toml").exists()
        || project_root.join("requirements.txt").exists()
        || project_root.join("setup.py").exists()
    {
        new_patterns.extend(&["__pycache__/", ".venv/", "venv/", "*.pyc", ".pytest_cache/"]);
    }
    // Unreal Engine: look for .uproject file.
    let has_uproject = std::fs::read_dir(project_root)
        .ok()
        .map(|entries| {
    ...
    {
        new_patterns.extend(&["build/", ".gradle/"]);
    }
    ...
    if new_patterns.is_empty() {
        // No recognized project type — skip .taignore generation but don't warn.
        return Ok(());
    }

    // Load existing patterns.
    let existing_content = if taignore_path.exists() {
        std::fs::read_to_string(&taignore_path).unwrap_or_default()
    } else {
        String::new()
    };

    let existing_patterns: std::collections::HashSet<&str> = existing_content
        .lines()
        .map(|l| l.trim())
        .filter(|l| !l.is_empty() && !l.starts_with('#'))
        .collect();

    // Only add patterns not already present.
    ...
    if to_add.is_empty() {
        println!("  [ok] .taignore already contains all recommended patterns.");
        return Ok(());
    }

    // Detect project label for the comment header.
    let project_labels: Vec<&str> = [
        (project_root.join("Cargo.toml").exists(), "Rust"),
        (project_root.join("package.json").exists(), "Node"),
        (project_root.join("go.mod").exists(), "Go"),
        (
            project_root.join("pyproject.toml").exists()
                || project_root.join("requirements.txt").exists(),
            "Python",
        ),
        (has_uproject, "Unreal Engine"),
        (
            project_root.join("build.gradle").exists()
                || project_root.join("build.gradle.kts").exists(),
            "Gradle",
        ),
        (project_root.join("pom.xml").exists(), "Maven"),
    ]
    .iter()
    ...
    let header = format!(
        "\n# Auto-generated by `ta setup vcs` — {} build artifacts\n# These directories are excluded from TA staging copies.\n",
        project_labels.join(", ")
    );

    let mut added_lines = header;
    for pattern in &to_add {
        added_lines.push_str(pattern);
        added_lines.push('\n');
    }

    let new_content = format!("{}{}", existing_content, added_lines);

    if dry_run {
        println!(
            "  [dry-run] Would add {} pattern(s) to .taignore ({} detected):",
            to_add.len(),
            project_labels.join(", ")
        );
        for p in &to_add {
        ...
        std::fs::write(&taignore_path, &new_content)?;
        println!(
            "  [updated] .taignore — added {} pattern(s) for {} project:",
            to_add.len(),
            project_labels.join(", ")
        );
        for p in &to_add {
...
"#,
    ...
    // ── .taignore generation tests ────────────────────────────────

    #[test]
    fn generate_taignore_rust_project() {
        let dir = TempDir::new().unwrap();
        std::fs::write(dir.path().join("Cargo.toml"), "[package]\nname = \"test\"").unwrap();

        generate_taignore(dir.path(), false).unwrap();

        let content = std::fs::read_to_string(dir.path().join(".taignore")).unwrap();
        assert!(content.contains("target/"), "should have target/ for Rust");
    }

    #[test]
    fn generate_taignore_node_project() {
        let dir = TempDir::new().unwrap();
    ...
    #[test]
    fn generate_taignore_go_project() {
        let dir = TempDir::new().unwrap();
    ...
    #[test]
    fn generate_taignore_python_project() {
        let dir = TempDir::new().unwrap();
        std::fs::write(dir.path().join("requirements.txt"), "flask==3.0.0").unwrap();
    ...
    #[test]
    fn generate_taignore_merges_with_existing() {
        let dir = TempDir::new().unwrap();
        std::fs::write(dir.path().join("Cargo.toml"), "[package]\nname = \"test\"").unwrap();

        // Write a pre-existing .taignore with target/ already present.
        std::fs::write(dir.path().join(".taignore"), "target/\nsecret/\n").unwrap();
        ...
        // Should have target/ exactly once (not duplicated).
        assert_eq!(content.matches("target/").count(), 1);
    }

    #[test]
    fn generate_taignore_dry_run_does_not_write() {
        let dir = TempDir::new().unwrap();
        std::fs::write(dir.path().join("Cargo.toml"), "[package]\nname = \"test\"").unwrap();

        generate_taignore(dir.path(), true).unwrap();

        // .taignore should NOT be created in dry-run mode.
        assert!(!dir.path().join(".taignore").exists());
    }

    #[test]
    fn generate_taignore_no_project_type_no_file() {
        let dir = TempDir::new().unwrap();
        // No recognized project files.
        generate_taignore(dir.path(), false).unwrap();
        // No .taignore should be created.
        assert!(!dir.path().join(".taignore").exists());
    }

    #[test]
    fn generate_taignore_unreal_project() {
        let dir = TempDir::new().unwrap();
